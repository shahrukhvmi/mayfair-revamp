...
const Step5 = () => {
    ...
    const { control, handleSubmit, setValue, watch, formState: { isValid } } = useForm({ mode: 'onChange' });
    ...
    return (
        <>
            ...
            <div className="min-h-screen flex items-center justify-center bg-[#FAFAF0] px-4 sm:px-6 lg:px-8">
                ...
                <FormWrapper width="my-20">
                    <ProgressBar percentage={50} />

                    <h1 className="text-2xl font-inter text-center mb-2 text-black">
                        Medical Questions
                    </h1>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-8">
                        {questions.map((q) => {
                            ...
                            return (
                                <div key={q.id} className={`p-5 shadow-sm border-1 rounded-md m-3 bg-white ${errorMessages[q.id] ? 'border-red-400' : 'border-gray-200'}`}>
                                    ...
                                    {errorMessages[q.id] && (
                                        <p className="text-sm text-red-500 mt-2">{errorMessages[q.id]}</p>
                                    )}
        ...
        ${isSelected ? "bg-green-50 border-violet-700" : "border-gray-300 bg-white hover:bg-gray-50"}`}
                                                >
                                                    ...
                                                    />
                                                    <div className={`w-5 h-5 rounded-sm border mr-2 flex items-center justify-center 
