const Product = ({
  ...
  return (
    ...
    >
      {/* Selected Badge */}
      {isSelected && (
        <div className="absolute top-3 left-3 z-30 bg-violet-500 text-white text-[10px] font-semibold px-3 py-1 rounded-full shadow-sm">
          Selected
        </div>
      )}
        ...
        <div>
          ...
          {lastOrderDate && (
            <p className="text-xs text-gray-500 mb-2">
              Last Ordered: {lastOrderDate}
            </p>
          )}
        </div>
        <span className="text-md text-primary bold-font mt-auto">
          {buttonText}
        </span>
      </div>
    </div>
