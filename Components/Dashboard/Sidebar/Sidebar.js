...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <div
      className={`m-3 sm:rounded-lg fixed top-0 left-0 lg:relative h-full bg-[#F9FAFB] py-4 px-3 flex flex-col shadow-md transform transition-transform duration-300 ease-in-out z-50 sm:relative sm:translate-x-0 sm:w-64 ${
    ...
    >
      ...
      <nav className="space-y-2">
        {navItems.map(({ href, label, icon, key }) => {
          const isActive = router.pathname.startsWith(href); // ✅ safer comparison
          return (
            <Link href={href} legacyBehavior key={key}>
              <a
                onClick={toggleSidebar}
                className={`flex items-center p-2 rounded-md ${key} ${
                  isActive
